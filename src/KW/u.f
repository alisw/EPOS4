...
#include "aaa.h"
      ...
      if(ish.ge.1)write(ifmt,'(a,i8,i3)')
     .'end of hacas - nptl icccptl = ',nptl,icccptl
      call checktime('exit hacas;') 
      write(ifmt,'(a,i10)')'maximum collision number',kcollmax
  ...
  !    id:  33100 ... phi -> K+ K- (BR 0.489)
  !         33101 ... phi -> K0 K0bar (BR 0.342)
  ...
  !   Am ende phi yield und spektra durch 0.83 teilen,
...
c spectator arrays
     ...
     +     frp0(nmax), frpx(nmax), frpy(nmax), frpz(nmax)
      ...
      integer iorptl(mxptl+29),idptl(mxptl+29),istptl(mxptl+29),
     *  ifrptl(2,mxptl+29),jorptl(mxptl+29),ibptl(4,mxptl+29)
     * ,ityptl(mxptl+29)
      real pptl(5,mxptl+29),tivptl(2,mxptl+29),xorptl(4,mxptl+29)
            ...
            if(abs(ityptmp).le.maxbar) then
              ...
              if(iutest.eq.1)then
                ...
                if(.not.(c.le.0..or.c.ge.0.))then
                  ...
                  stop'ERROR NaN catch uinitial (1)'
                endif
            ...
            if(abs(ityptmp).ge.minmes) then
              ...
              if(iutest.eq.1)then
                ...
                if(.not.(c.le.0..or.c.ge.0.))then
                  ...
                  stop'ERROR NaN catch uinitial (2)'
                endif
                if(pptl(4,nn).lt.2.)then
                  a=pptl(4,nn)**2-pptl(3,nn)**2
                  b=pptl(5,nn)**2+pptl(1,nn)**2+pptl(2,nn)**2
...
c     *        ,rx(itmp),ry(itmp),rz(itmp),px(itmp),py(itmp),pz(itmp)
      ...
      npart = nbar + nmes
      if(ish.ge.2)write(ifch,*)' '
      write(ifmt,*)'hacas nr of ptls entering: ',npart
...
#include "aaa.h"
      do nn=nptlpt+1,nptlbd
        if(istptl(nn).eq.0)then
          if(abs(idtmp).lt.11.or.abs(idtmp).gt.16)then
            if(xorptl(4,nn).lt.-0.5)then
              write(ifch,'(a,2i8,i4,5f10.2)')
     .       ' HHHHHHH   affini    ',nn,idptl(nn),ityptl(nn)
     .       ,sqrt(pptl(1,nn)**2+pptl(2,nn)**2)
     .       , xorptl(4,nn),xorptl(3,nn),xorptl(1,nn),xorptl(2,nn)
             endif
           endif
        endif
      enddo
...
ckw      read(9,99) flag,inputstr
      ...
      elseif(line.eq.5)then
        ...
        write(inputstr(1:6),'(i6)')nint(engy)
...
c------------------------------------------------------------------------
      ...
      !  ik3 ... first daughter ityp code
      !  ik4 ... first daughter iso3 code
      ...
      !-------------------------------------------------
       ...
       if(jk1.eq.109)then
         if(jk3.eq.106.and.jk4.eq. 1)ka=33100 !phi -> K+ K- (0.489)
         if(jk3.eq.-106.and.jk4.eq.-1)ka=33100 !phi -> K+ K- (0.489)
         if(jk3.eq.106.and.jk4.eq.-1)ka=33101 !phi -> K0 K0bar (0.342)
         if(jk3.eq.-106.and.jk4.eq. 1)ka=33101 !phi -> K0bar K0 (0.342)
       endif
     ...
     $      phinum,phiscatt,phiidres
      ...
      if(ninit.eq.1)then
       if(nexit.eq.2)then !consider only two ptl decays
        ...
        ik3=ityp(inew(1))   !first daughter ityp code
        ik4=iso3(inew(1))   !first daughter iso3 code
        call getResoId(ik1,ik2,ik3,ik4,ka)
       endif
       if(ka.ne.0)then
        phinum=phinum+1
        if(phinum.gt.maxphi)stop'#####  ERROR 25092014 #####'
        phiidres(phinum)=ka
        phipx(phinum)=pxii(1)
        phipy(phinum)=pyii(1)
        phipz(phinum)=pzii(1)
      ...
      if(ninit.eq.2)then
        do i=1,2
          if(phiidii(i).gt.0)then
            !print*,'decay from marked:'
            !.         ,indii(i),itypii(i),iso3ii(i),phiidii(i)
