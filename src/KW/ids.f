...
c-----------------------------------------------------------------------
      ...
      call getidptl(j,idj)
      idjj=idj
      call getiorptl(j,iorj) !parent
  77  continue
      if(iorj.gt.0)then
        idorj=ideposf( 2 ,iorj)
     ...
     &          idora.eq.350 .or.idora.eq.450)NoLongLivParent=.false.
          ...
          call getistptl(iorj,istiorj)
          if(istiorj.eq.1.or.istiorj.eq.6)then   !parent=hadron
            call getiorptl(iorj,iorj2)  !grandparent  
            iorj=iorj2
...
#include "aaa.h"
      ...
      double precision s,puv,pdv,psv,pcv,pbv,pus,pds,pss,pcs,pbs,piso !bg
     * ,proba
      character c*1
 ...
 10    format('entry idraflx, j,x,q: ',a1,i2,2g13.5,/
     &  ,15x,'jc :',2(1x,6i2),/,14x,'jcv :',2(1x,6i2))
       ...
       if(c.eq."v")then
         ...
         pss=dble(jc(3,j))-psv
         pcs=dble(jc(4,j))-pcv
         pbs=dble(jc(5,j))-pbv !bg
       endif

      if(ish.ge.8)then
        write(ifch,'(a,5f6.3)')'idraflx valence:',puv,pdv,psv,pcv,pbv !bg
        write(ifch,'(a,5f6.3)')'idraflx sea:',pus,pds,pss,pcs,pbs     !bg
      endif

       qq=0.
       if(iso.gt.0)then
         if(icl.eq.2)puv=puv*0.5d0   !because GRV already take into account the fact that there is 2 u quark in a proton
         psea=psdpdf(xxx,qqs,qq,icl,-1)
         puv=puv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,1)-psea))
         pus=pus*dble(psea)
         psea=psdpdf(xxx,qqs,qq,icl,-2)
         pdv=pdv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,2)-psea))
         pds=pds*dble(psea)
       elseif(iso.lt.0)then
         psea=psdpdf(xxx,qqs,qq,icl,-2)
         puv=puv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,2)-psea))
         pus=pus*dble(psea)
         if(icl.eq.2)pdv=pdv*0.5d0
         psea=psdpdf(xxx,qqs,qq,icl,-1)
         pdv=pdv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,1)-psea))
         pds=pds*dble(psea)
       else
         psea=.5d0*(psdpdf(xxx,qqs,qq,icl,-1)+psdpdf(xxx,qqs,qq,icl,-2))
         piso=max(0d0,dble(psdpdf(xxx,qqs,qq,icl,1)-psea)
     &               +dble(psdpdf(xxx,qqs,qq,icl,2)-psea))
       ...
       psea=psdpdf(xxx,qqs,qq,icl,-3)
       psv=psv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,3)-psea))
       pss=pss*dble(psea)
       if(nrflav.ge.4)then
         psea=psdpdf(xxx,qqs,qq,icl,-4)
         pcv=pcv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,4)-psea))
         pcs=pcs*dble(psea)
       else
         pcv=0d0
         pcs=0d0
       endif
       if(nrflav.ge.5)then                !bg
         psea=psdpdf(xxx,qqs,qq,icl,-5)
         pbv=pbv*max(0d0,dble(psdpdf(xxx,qqs,qq,icl,5)-psea))
         pbs=pbs*dble(psea)
       else                              !bg
         pbv=0d0
         pbs=0d0
      ...
      if(ish.ge.8)then
        write(ifch,'(a,5f6.3)')'idraflx P(valence):',puv,pdv,psv,pcv
     *  ,pbv !bg
        write(ifch,'(a,5f6.3)')'idraflx P(sea):',pus,pds,pss,pcs,pbs !bg
      endif

      s=puv+pdv+psv+pcv+pbv+pus+pds+pss+pcs+pbs !bg
      if(s.gt.0.)then
       r=rangen()*s
       if(r.gt.(pdv+pus+pds+pss+psv+pcv+pcs+pbv+pbs).and.puv.gt.0.)then!bg
        i=1
        jcval(i,j)=jcval(i,j)-1
        proba=puv
       elseif(r.gt.(pus+pds+pss+psv+pcv+pcs+pbv+pbs).and.pdv.gt.0.)then!bg
        i=2
        jcval(i,j)=jcval(i,j)-1
        proba=pdv
       elseif(r.gt.(pds+pss+psv+pcv+pcs+pbv+pbs).and.pus.gt.0.)then !bg
        i=1
        proba=pus
       elseif(r.gt.(pss+psv+pcv+pcs+pbv+pbs).and.pds.gt.0.)then !bg
        i=2
        proba=pds
       elseif(r.gt.(psv+pcv+pcs+pbv+pbs).and.pss.gt.0.)then !bg
        i=3
        proba=pss
       elseif(r.gt.(pcv+pcs+pbv+pbs).and.psv.gt.0.)then !bg
        i=3
        jcval(i,j)=jcval(i,j)-1
        proba=psv
       elseif(r.gt.(pcs+pbv+pbs).and.pcv.gt.0.)then !bg
        i=4
        jcval(i,j)=jcval(i,j)-1
        proba=pcv
       elseif(r.gt.(pbv+pbs).and.pcs.gt.0.)then !bg
        i=4
        proba=pcs
       elseif(r.gt.pbs.and.pbv.gt.0.)then !bg
        i=5                               !bg
        jcval(i,j)=jcval(i,j)-1
        proba=pbv                         !bg
       elseif(pbs.gt.0.)then !bg
        i=5                  !bg
        proba=pbs            !bg
       else
...
c          c='s' : from sea
c          c='r' : from sea (always without c quarks and b quarks)
c          c='d' : from sea for second quark in diquark
c          c='c' : take out c quark first
c          c='b' : take out b quark first    !bg
c             jc : quark content of remnant
c     j=1 quark, j=2 antiquark,
...
c      write(ifch,*)'idrafl',pu,pd,ps
      ...
      if(s.gt.0.)then
       r=rangen()*s
       if(r.gt.(pu+pd+ps+pc).and.pb.gt.0d0)then !bg
        i=5                                     !bg
       elseif(r.gt.(pu+pd+ps).and.pc.gt.0d0)then
        i=4
...
c          call idsufl3(i,j,jc)
cc   be sure that jc is not empty
c          if(jc(i,j).eq.0)then
c            call idenco(jc,ic,iret)
c            if(iret.eq.0.and.ic(1).eq.0.and.ic(2).eq.0)then
c              jc(i,j)=jc(i,j)+1
c              jc(i,3-j)=jc(i,3-j)+1
c              iretso=1
c            endif
c          endif
c        elseif(iremn.eq.3)then
        if(iremn.ge.2)then
          call idsufl3(i,j,jc)
        else
...
#include "aaa.h"
      ...
      if(s.gt.0.)then
       r=rangen()*s
       if(r.gt.(pu+pd+ps+pc).and.pb.gt.0d0)then
        i=5                                  !bg
       elseif(r.gt.(pu+pd+ps).and.pc.gt.0d0)then
        i=4
       elseif(r.gt.(pu+pd).and.ps.gt.0d0)then
...
c-----------------------------------------------------------------------
      subroutine idtr4(idi,ic) !id -> ic via idresi.dt (for string decay) 
c-----------------------------------------------------------------------
#include "aaa.h"
      parameter (mxindx=9900,mxremax=100,mxmamax=30,mxmxmax=30)
      common/crema/indy(mxindx),rema(mxremax,mxmamax),
     ...
     *,mxre,mxma,mxmx 
      ...
      ic(1)=000000
      ic(2)=000000
      if(mod(abs(id),100).eq.88)return !not a particle
      ...
      if(iabs(id).lt.1e8)then
        ...
        jj=0
        ima=1
        do while(ima.lt.mxma.and.jj.eq.0)
          ima=ima+1
          if(iabs(id).eq.idre(ii,ima))jj=ima
        enddo
        if(id.gt.0)then
          ic(1)=icre1(ii,jj)
          ic(2)=icre2(ii,jj)
...
c      write(ifmt,*)'idtr4 ---->  id: ',id,ic
      return

 9998   continue
        write(ifmt,*)'id: ',id
        icrash=0
        dmy=1/icrash !to force crash
        stop'in idtr4'
...
c     *,330000,000000,  17   ,450000,000000,  18   ,660000,000000,  19
...
c     read the table src/KWt/ids.dt which contains all id 
...
#include "aaa.h"
      ...
      write(ifmt,'(2a)')'load ',fnnx(1:nfnnx)//'/src/KWt/idt.dt'
      open (95,file= fnnx(1:nfnnx)//'/src/KWt/idt.dt'
     *,STATUS='UNKNOWN')
      nline=0
...
c-----------------------------------------------------------------------
      subroutine testNewParticleList
c-----------------------------------------------------------------------
#include "aaa.h"
      character dname*30
      double precision das,das2
      !load old list
      write(ifmt,'(a)')'load ptltab, dkytab from '
      write(ifmt,'(1x,a)')fnnx(1:nfnnx)//'YKt/ptl7.data'
      write(ifmt,'(1x,a)')fnnx(1:nfnnx)//'YKt/dky7.data'

      stop'comment this stop in order to run the test' 
 
      !uncomment the following call in order to run the test

c      call loadptldky(
c     . fnnx(1:nfnnx)//'YKt/ptl7c.data'//CHAR(0) ,
c     . fnnx(1:nfnnx)//'YKt/dky7.data'//CHAR(0) ) 
      write(ifmt,'(80a)')('#',i=1,80)
      write(ifmt,*)' Nr idEPOS   idPDG     Charge    BaryonNr '
     .,'Strangeness Degeneracy    Flag' 
      last=0 
      n=0 
      do 
       call getNextParticle
     . (dname,das,iQ,id,igs,js,iB,iS,last,iflag)
       if(iflag.gt.0)then 
        n=n+1
        idpdg=idtrafo('nxs','pdg',id)
        !write(ifmt,*)'NEW',n,id,idpdg
        !~~~OLD~~~
        last2=0
        n2=0
        do
        call 
     .  igetparticle(dname,das2,iQ2,id2,igs2,js2,iB2,iS2,last2,iflag2)
        if(abs(last2).eq.1)then
          n2=n2+1
          !write(ifmt,*),'   OLD',n2,id2
          if(idpdg.eq.id2)then
            if(iQ.ne.iQ2.or.iB.ne.iB2.or.iS.ne.iS2
     .       .or.igs.ne.igs2.or.iflag.ne.iflag2)then
             write(ifmt,'(i4,i7,i8,5(3x,2i4))')
     .       n,id,idpdg,iQ,iQ2,iB,iB2,iS,iS2,igs,igs2,iflag,iflag2
            endif 
            goto 998
          endif
        endif
        if(last2.lt.0)then
          write(ifmt,'(2a,i6,i8)')'No particle found corresponding to  '
     .    ,'idEPOS idPDG =',id,idpdg
          goto 998
        endif
        enddo
  998   continue 
       endif
       if(last.eq.-1) goto 999
      enddo
  999 continue
      write(ifmt,'(a,i5)')'Number of particles chosen:',n  
      stop'testNewParticleList finished'
      end

c-----------------------------------------------------------------------
c subroutine writeidtables 
c removed in 3248 
c this routine was only useful (at some stage) to transform old to new id table
c-----------------------------------------------------------------------
...
c Complete rewrite of the table by !gs
...
c  the data part as table can be found in epos3209/ids_idresi.f
c  The corresponding table is src/KWt/idresi.dt_3208
c  The extended table is src/KWt/idresi.dt
c-----------------------------------------------------------------------
...
#include "aaa.h"
     ...
     *,mxre,mxma,mxmx 
      ...
      write(ifmt,'(2a)')'load ',fnnx(1:nfnnx)//'src/KWt/idresi.dt'
      open (98,file= fnnx(1:nfnnx)//'src/KWt/idresi.dt')
