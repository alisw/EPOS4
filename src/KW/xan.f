...
c...........check ids
     ...
     &          .and.(ityptl(j).eq.80.or.ityptl(j).eq.81))then!resonances marked in u.f
          ...
          enddo
          if(ish.ge.10)write(ifch,*)n,j,istuse(n)
     &     ,'      ist,id',istptl(j),idepos,'    ',go
...
c.................................................................
     ...
     &            /zbbin(l,nac(n),loo)
      ...
      sigma=xsectionpar()
      if(ntevt.ne.0)
     &   sigma=10.*pi*bmax**2.*nfull/ntevt !total (untriggered) sigma
      if(norm2.eq.3)then      !differential (triggered) sigma
        !???????????????check nfull nfreeze stuff ???????????????????????
        if(ntevt.ne.0)
...
c        pmax=sqrt((engy/2)**2-prom*2)
     ...
     $           pt.gt.0)then
      ...
      elseif(inom.eq.75)then    !'v2'
        if(iranphi.ne.1)stop'##### ERROR 29062010b #####'
        aa=cos(phinll)
        bb=sin(phinll)
        cc=-sin(phinll)
        dd=cos(phinll)
...
c        write(ifmt,*)'ici',x,min(Esum-Psum,Esum+Psum)/ecms
      ...
      elseif(inom.eq.360)then                       
        x=nint(typevt)            !ND(1), DD(2), or SD(3)
      ...
      elseif(inom.eq.551)then                       ! ...zevt'  
        if(icas.lt.0)then
          ...
          case(3)
            ...
            enddo
          case(4)
            call eventvariget(4,x)                 !z4zevt = Nhpom
          case(10)
            call eventvariget(10,x)                !z10zevt = Conn1
          case(11)
            call eventvariget(11,x)                !z11zevt = Conn2
          case(20)
            call eventvariget(20,x)                !z20zevt = BinFac
          case(21)
            call eventvariget(21,x)                !z21zevt = BinFacFit
          case default   
            stop'ERROR 210115'
          end select
        endif
...
#include "xan.h"
     ...
     *           .and.pt .lt.xpara(ishift+4,n)   )sum=sum+val
            endif
          endif
        endif
...
c                  rap=0.5*log((p4+p3)/(p4-p3))  !always correct but numerically unstable
  ...
  78                  i25=i25-1
     ...
     .     ,mxphi,0.,phimx,'#eta   ','#phi   ','E_{T} for '
     . ,'hadrons from hardest process       ')
          call oo2bin(-4, mxxeta,mxxphi,bmtx,mxeta,etamin,etamax
     .     ,mxphi,0.,phimx,'#eta   ','#phi   ','E_{T} for '
     . ,'all final hadrons                  ')
        endif
      endif
  ...
  79            ii=ii+1
     ...
     .           bmtx(ieta,iphi)=bmtx(ieta,iphi)+pt
        ...
        if(iorap.gt.1.and.nj.gt.0)then
          call oo2bin(-4, mxxeta,mxxphi,amtx,mxeta,etamin,etamax
     .     ,mxphi,0.,phimx,'#eta   ','#phi   ','E_{T} for '
     .     ,'direct partons from hardest process')
          call oo2bin(-4, mxxeta,mxxphi,bmtx,mxeta,etamin,etamax
     .     ,mxphi,0.,phimx,'#eta   ','#phi   ','E_{T} for '
     .     ,'final partons from hardest process ')
        endif
      endif
      !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     .  ' +++jet i pt, phi, eta',i,pt, phi, eta
      ...
      enddo
      !++++++++++++++++++++++++++++check++++++++++++++++++++++++++++++
      if(jcheck.eq.1.and.kcheck.eq.1)
     .call oo2bin(-4, mxxeta,mxxphi,bmtx,mxeta,etamin,etamax
     . ,mxphi,0.,phimx,'#eta   ','#phi   ','          '
     . ,'excluded towers (D>5)              ')
      !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
