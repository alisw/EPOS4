...
#include "aaa.h"
     ...
     *'enter treestore'
     ...
     .                    fndt(1:ii)//'.root'//CHAR(0))
          ...
          if(hepmc.eq.1)then
            write(ifmt,'(a)')'openhepmc '//fndt(1:ii)//'.hepmc'
            call openhepmc(fndt(1:ii)//'.hepmc'//CHAR(0))
          endif 
          izm=ifillTree
          nfullt(1)=nfull
     ...
     .     ,numpom,nfullt(numpom)
          ...
          if(hepmc.eq.1)
     .         call openhepmc(fn(1:nfn)//'.hepmc'//CHAR(0))
          izm=izmode
        endif
        iversnx=iversn
        laprojx=laproj
...
#include "aaa.h"
      ...
      if(hepmc.eq.1)
     .     call openhepmc(fndt(1:(nfndt-5))//'.hepmc'//CHAR(0))
      do i=i3,i4
        call clop(3)
        call updateName(i)
        if(rootfile(i-i1+1))then
