...
C
      ...
      parameter (kollmx=43000/mmrye,npommx=150)
      parameter (mamx=210/mmryk,ntynd=1,ntymi=0,ntymx=3)
      parameter (nflavems=5)

!----------------------------------------------------------------------
! POMERON
!----------------------------------------------------------------------

      double precision om1MeanCol,om1intc,sigmak
     *        ,atilde,btildep,btildepp
      common/comega/om1MeanCol(2,0:kollmx),sigmak(2,kollmx)
     *,om1intc(kollmx)
      common/comtilde/atilde(ntymi:ntymx,kollmx)
     *,btildep(ntymi:ntymx,kollmx)
     *,btildepp(ntymi:ntymx,kollmx)
     *,ntymin,ntymax
      double precision atildp,btildpp,btildppp
      common/comtilde2/atildp(ntymi:ntymx,npommx,kollmx)
     *,btildpp(ntymi:ntymx,npommx,kollmx)
     *,btildppp(ntymi:ntymx,npommx,kollmx)

!----------------------------------------------------------------------
! POMERON CONFIGURATION
!----------------------------------------------------------------------
   
      double precision wwnk
        common/cems9/wwnk(0:2,npommx,kollmx),q2kmin(2,0:npommx,kollmx)
c     *,binscalk(kollmx),nxmaxq2(kollmx)  
      common/col/coord(4,kollmx),koll,iproj(kollmx),itarg(kollmx)
     ...
     *,ssigp,ssigt
c     common/col5/expb2par(2,npommx,kollmx),hnormp,hnormt,snormp,snormt
      common/col5/expb2par(1,1,1),hnormp,hnormt,snormp,snormt
     *,hsigp,hsigt,ssigp,ssigt
      common/col6/gammaV(kollmx),epsilongp(-1:ntymx,kollmx)
     *,epsilongt(-1:ntymx,kollmx),epsilongs(-1:ntymx,kollmx)
c     *,epsilongf(kollmx),epsilongb(kollmx)
     *,kproj3(mamx,mamx),ktarg3(mamx,mamx),lproj3(mamx),ltarg3(mamx)
      common/col4/bk(kollmx),bkx(kollmx),bky(kollmx)
c     *,bhpr(npommx,kollmx)
c      double precision PomInck,xxnuc,xzcutpar
c      common/col7/PomInck(kollmx),xzcutpar(kollmx)
c     *,zparpnx(kollmx),zpartnx(kollmx)
c     *,xxnuc(mamx,2,kollmx),npnuc(mamx,2,kollmx)
c     *,irnuc(mamx,2,kollmx),knucnt(2,kollmx)
      real zparpro(-1:2,kollmx),zpartar(-1:2,kollmx)
     *,zparpnx(0:2,kollmx),zpartnx(0:2,kollmx)
      common/col7/zparpro,zpartar,zparpnx,zpartnx
...
!----------------------------------------------------------------------
     ...
     *,nvpr(npommx,kollmx),nbkpr(npommx,kollmx)
      ...
      common/cems2b/konnSave(2,npommx,kollmx),connSave(2,npommx,kollmx)
...
!----------------------------------------------------------------------
      ...
      double precision xpr,ypr,xppr,xmpr
     &,xpprlad,xmprlad
      common/cems1/xpr(npommx,kollmx),ypr(npommx,kollmx)
      common/cems1c/xppr(npommx,kollmx),xmpr(npommx,kollmx)
      common/cems1d/xpprlad(npommx,kollmx),xmprlad(npommx,kollmx)
      common/cems1e/jsplitpom(npommx,kollmx),q2bor(npommx,kollmx)
     & ,nemispr(2,npommx,kollmx),shatpr(npommx,kollmx),fxsplit,estpom
...
! xm2pr(n,k):  x of string end m2                               \/    v = valence (anti)quark
! idp1pr(n,k): id of string end p1 ( 0=0         )              /\    w = valence (anti)diquark (2 v quarks)
! idp2pr(n,k): id of string end p2 ( 1=s   2=v   )             /  \   s = sea (anti)quark
! idm1pr(n,k): id of string end m1 ( 4=d   5=w   )            /    \  d = sea (anti)diquark 
! idm2pr(n,k): id of string end m2 ( 8=f         )           m1    m2
! xxp1pr(n,k),xyp1pr(n,k): px,py of string end p1
! xxp2pr(n,k),xyp2pr(n,k): px,py of string end p2               left = quark 
! xxm1pr(n,k),xym1pr(n,k): px,py of string end m1               right = antiquark
...
!---------------------------------------+-------------------------------(ProSeTy)
! idsppr(n,k): flav soft P (n,k) (proj) |0=ss not linked to prj/trg ;  <0=diffr centrale ; 100 = val dur               
! idstpr(n,k): flav soft P (n,k) (targ) | otherwise: 1=ss,2=vs|sv,4=sd|ds|dd,5=sw|ws (see idp1pr etc)
!                                       |   case of hard +5
!---------------------------------------+------------------------------
...
!----------------------------------------------------------------------
      double precision xnpp,xnpt,xnpo
      common/cemsr2/xnpp(kollmx),xnpt(kollmx),xnpo(kollmx)
      common/cemsr3/kolp(mamx),kolt(mamx)!,npp(kollmx),npt(kollmx)
...
! npp(k),npt(k): for a pair k, number of cut Pomerons for proj and targ with "large" x
...
!----------------------------------------------------------------------
     ...
     &,delxi,ddelx,r3pomti
      ...
      common/cemsr13/xis,delxi,ddelx,r3pomti,ucfpro,ucftar
