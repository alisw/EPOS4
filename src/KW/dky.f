...
c-----------------------------------------------------------------------
c
c  uses overflow procedure based on the cccptl C++ object
c   extending thus the list of particles beyond mxptl
c
c-----------------------------------------------------------------------
...
#include "aaa.h"
      ...
      if(i.gt.mxptl)then
         if(icccptl.eq.0)then
            write(ifmt,*)'hdecas: i>mxptl & icccptl=0     ',i,nptl
            stop'####### ERROR 10032017 #######'
         endif
         call restorecccptl(i,mxptl+2)
         itmp=mxptl+2
         nptl=mxptl+2     
      endif
      nptl=min(nptl,mxptl+2)
      n1=nptl+1
      
      call hdecax(itmp,iret)
      n2=nptl
      if(n2.ge.n1)then
      ifrptl(1,itmp)=nptlsave+1
      ifrptl(2,itmp)=nptlsave+1+n2-n1
      else
      ifrptl(1,itmp)=0
      ifrptl(2,itmp)=0
      endif
      
      if(i.gt.mxptl)call dumpcccptl(mxptl+2,i)
     
      nptl=nptlsave
      do n=n1,n2
        iorptl(n)=i
        nptl=nptl+1
        if(n.eq.nptl.and.n.le.mxptl)then
          continue
        elseif(nptl.gt.mxptl)then
          call checkcccptl(nptl)
          call dumpcccptl(n,nptl)
        else
          stop'########## ERROR 14082014 ###########'
        endif  
      enddo

      if(ish.ge.4)write(ifch,'(a,i8,i8,i3)')
     .'end of hdecas - i nptl icccptl = ',i,nptl,icccptl
...
c-----------------------------------------------------------------------
      subroutine hdecax(i,iret)
c-----------------------------------------------------------------------
c
c   we use /cptl/ up to nptl = mxptl+29
c     needed for overflow procedure using the cccptl C++ object
c
c-----------------------------------------------------------------------
c  decay of particle i  (main decay routine)
c-----------------------------------------------------------------------
...
c entry
      ...
      call utpri('hdecax',ish,ishini,5)
      ttauz=ttaus
...
c small droplet decay
      ...
      if(iabs(idptl(i)).gt.1e8)then
        ...
        stop'hdecax: no longer supported (2).       '
      endif

c  ordinary decay

      if(ityptl(i).eq.61)stop'ERROR 30062021'  !old method using old particle table 
...
c loop over decay products
      ...
      !~~~~~~~~~~~~~~~~~~
      tivptl(1,n)=ti
      if(ityptl(i).eq.61)stop'ERROR 30062021b'
...
1000  continue
      ttaus=ttauz
      call jtauin
      call utprix('hdecax',ish,ishini,5)
      return
      end
...
c   we use /cptl/ up to nptl = mxptl+29
c
c     needed for new overflow procedure using cccptl C++ object
c
c-----------------------------------------------------------------------
...
c      if(model.eq.4.and.iappl.eq.7)then
c        if(abs(idptl(ipp)).gt.13.and.abs(idptl(ipp)).ne.1120
c     &.and.abs(idptl(ipp)).ne.15)call decaymod(ipp,iret)
c        if(iret.gt.0)goto 1000
c        naddptl=0
c        goto 900 
c      endif
...
c      nstart=nptl+1                  !?????????????????unused
      ...
      do 110 i=1,5         !store id and mass of products
        ...
        if(nptl+naddptl+1.gt.mxptl+29) goto 9999
...
c save mass (and energy) as used for decay (mass shift due to lifetime)
        ...
        pptl(5,ip)=pgen(5,1)
        nptl=nptl+naddptl
        if(nptl.gt.mxptl+29)call utstop('hdecay: nptl>mxptl+29&')
c        nqk=0           !???????????????????unused
...
c     -----
  ...
  140 format(/' ----------------------------'/
     *'    decay  at tau =',f6.2/
     *' ----------------------------')
      write(ifch,*)'indices may be temporary, when exceeding mxptl!!!'
      write(ifch,*)'decaying object:'
      call dlist(ip,ip)
      write(ifch,*)'decay products:'
      call dlist(nptlb+1,nptl)
      endif
      if(ish.ge.5)then
...
c     ----
 ...
 9999   call utstop('hdecay: mxptl+29 too small&')
        end

      subroutine dlist(n1,n2)
      !does not use particle overflow features (contrary to alist)
#include "aaa.h"
      do i=n1,n2 !(instead of do iloo=n1,n2 ...)
        ...
        write(ifch,'(1x,i7,i7,i7,i7,i7,i10,2i3,2x,3(e9.3,1x))')
     &          iorptl(i),jorptl(i),i,ifrptl(1,i),ifrptl(2,i)
     &    ,idptl(i),istptl(i),ityptl(i),ptptl,pptl(5,i),pptl(4,i)
      enddo
      end
...
#include "aaa.h"
      ...
      write(ifmt,'(2a)')'load ',fnnx(1:nfnnx)//'src/KWt/idky5.dt'
      open(95,file= fnnx(1:nfnnx)//'src/KWt/idky5.dt'
     .,STATUS='UNKNOWN')
