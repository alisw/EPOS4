...
C
      ...
      double precision xx
      psevi0=q1+qq+xx+m+l
      stop'in qsh.f : needs update'
      end

      function psev0(qj,qq,xx,k)
      double precision xx
      psev0=qj+qq+xx+k
      stop'in qsh.f : needs update'
      end

      function psev(qj,qq,xx,m,k,n)
      double precision xx
      psev=qj+qq+xx+m+k+n
      stop'in qsh.f : needs update'
      end
...
c------------------------------------------------------------------------
      function EsoftQZero(zz,qq)    ! former psftilf
c-----------------------------------------------------------------------
c
...
c
c   EsoftQZero = int(dz) z^(1+dels) * (1-zz/z)^betpom * P_qG(z)
c
c-----------------------------------------------------------------------
      double precision zmin,z,zz
...
#include "sem.h"
     ...
     *  **(1.d0/(1.d0+dels(1)))
        EsoftQZero=EsoftQZero+a1(i)*max(1.d-15,(1.d0-zz/z))**betpom(1)
     *  *(z**2+(1.-z)**2)
      enddo
      enddo
      EsoftQZero=EsoftQZero*float(noflav(qq))
     *          *0.5*(1.d0-zmin)/(1.+dels(1))!1.5=naflav/2 at Q0
      return
      end
...
c###########################################################################
      ...
      !  meaning of      q 2 m i n       changed !!!!!!!!!!
       
      !  therefore in epos-qsh renamed into q2dmin !!!!!!!
      
      !  preliminary set to -99999 in epos-bas !!!!!!!!!
      
      ! psevi0,  psevi  use  tables + explicitely   q 2 c m i n

      !  check psjti calls !!!!!!!!!
      ! A new first argument has been added to the function
      ! In qsh.f the first argument is always set to 1 without 
      !    verification 


      !-----------------------------------------------------
      !  reshuffled from sem, sto, sha, uti
      !
      !    contains DIS, and unused 3P stuff
      !             ---      ---------------
      !-----------------------------------------------------
...
c      double precision om5p,xh,yh,v3pom(4),om2p
      ...
      if(iappl.ne.8)return
      
      q2jmin=q2dmin       !TP temporary entries to check f2 without saturation
      q2cmin(1)=q2dmin
      q2cmin(2)=q2dmin
      call ipoBasics(q2cmin) 
      call ipoCSzeroTables(q2cmin)
      call ipoOm5Tables(1) !TP end temporary
      ...
      if(lcalc)then
        ...
        read (1,*)qcdlam0,q2jmin0,q2ini0,naflav0,epmax0,edmax0
        if(qcdlam0.ne.qcdlam)write(ifmt,'(a)')'inidi: wrong qcdlam'
        if(nequal(q2jmin0,q2jmin))write(ifmt,'(a)')'inidi: wrong q2jmin'
     ...
     *  .or.edmax0 .ne.edmax)then
           ...
           write(ifmt,'(a//)')'   we ignore this for the moment!!!'
           !stop
        endif
c        read (1,*)csdsi,csds,csdt,csdr
        do j=1,21
          do m=1,2              !parton type at the end of the ladder
            do k=1,26
c              write (*,*)'cxdsi,j,m,k',j,m,k
        ...
        do j=1,21
          do m=1,2
            do k=1,26
c              write (*,*)'csds,j,m,k',j,m,k
              do ilong=1,2
                read (1,*)csds(j,k,m+2*(ilong-1))
              enddo
              read (1,*)csdt(j,k,m),csdr(j,k,m)
            enddo
          enddo
        enddo
        close(1)
      ...
      write (1,*)qcdlam,q2jmin,q2ini,naflav,epmax,edmax

      do j=1,21
        qq=q2jmin*exp(.5*(j-1))                !photon virtuality

        do m=1,2               !parton type at the end of the ladder
          q2mass=qcmass**2
          s2min=4.*max(q2mass,q2jmin)+qq
...
c----------------------------------------------------------
      subroutine iniFakeEA  !fake e-A with pi0-A for hadron production
c----------------------------------------------------------
#include "aaa.h"
         !fake e-A with pi0-A for hadron production
        if(inicnt.eq.1)call phoGPHERAepo(0)  !kinematic initialization
        idproj=110
        if(model.eq.2.or.model.eq.7)then      !qgsjet01/II can not run pi0 as projectile
          idproj=120*(1-2*int(rangen()+0.5d0))
        endif
        laproj=-1
        maproj=1
        engy=-1
...
1     continue
      if(iolept.eq.1)then
        ...
        gdv01=psdh(ydmax*wtot,qdmin,iclpro,0,0)
        gdv02=psdh(ydmax*wtot,qdmin,iclpro,0,1)
        gds01=psdsh(ydmax*wtot,qdmin,iclpro,dqsh,0,0)
        gds02=psdsh(ydmax*wtot,qdmin,iclpro,dqsh1,0,1)
...
2       continue
        ...
        gdv(1)=psdh(wd,qq,iclpro,0,0)
        gdv(2)=psdh(wd,qq,iclpro,0,1)
        gds(1)=psdsh(wd,qq,iclpro,dqsh,0,0)
        gds(2)=psdsh(wd,qq,iclpro,dqsh1,0,1)
...
21      call lepexp(xbjevt,qsq)
      ...
      if(long.eq.0)then
        ...
        sdmin=4.*max(q2dmin,qcmass**2)+qq  !minimal mass for born
        xmm=(5.*sdmin-qq)/4.
     ...
     *(long.eq.0.or.wd.gt.sqmin))then
        if(long.eq.0)then
          xd=qq/wd
          tu=psdpdf(xd,q2dmin,0.,iclpro,1)/2.25
          td=psdpdf(xd,q2dmin,0.,iclpro,2)/9.
          gdv0=(tu+td)*4.*pi**2*alfe/qq
     *    *sngl(psuds(qq,1)/psuds(q2dmin,1))
     ...
     *      'qq,egyevt,iq1',qq,egyevt,iq1
            en1=0.5*egyevt
            en2=en1
            call timsh2(qq,0.,en1,en2,iq1,-iq1,iq1,-iq1
     *           ,ey0,s0xi,c0xi,s0i,c0i  ,s0xh,c0xh,s0h,c0h,0,0,0,0,0)
            ...
            s0xh=0.
            c0xh=1.
            call psreti(nqc,jq,1,ey0,0.,1.,0.,1.,s0xh,c0xh,s0h,c0h,iret)
            goto 17
          endif
        endif
...
3       continue
     ...
     *  **rangen()+qq/wd
        ...
        tu=psdpdf(xd,q2dmin,0.,iclpro,1)
        td=psdpdf(xd,q2dmin,0.,iclpro,2)
     ...
     *    'xpmax*wd,qq,sds0,sdn0,sdb0,sdt0,sdr0:'
          if(ish.ge.3)write (ifch,*)
     *    xpmax*wd,qq,sds0,sdn0,sdb0,sdt0,sdr0
        gb10=sdt0*fzeroGluZZ(0d0,1)+(sdnq0+sdtq0)
     *        *FzeroQuaZZ(0d0,1)
          gb10=gb10*15.
...
4         xd=xdmin*(xpmax/xdmin)**rangen()
          ...
          if(ish.ge.3)write (ifch,*)xd*wd,qq,sds,sdn,sdb,sdt,sdr
          wwg=sdt*fzeroGluZZ(dble(xd),1)
          wwq=(sdnq+sdtq)*FzeroQuaZZ(dble(xd),1)
          gb12=(wwq+wwg)/gb10*(xpmax/xd)**dels(1)
          if(gb12.gt.1..and.ish.ge.1)write(ifmt,*)
     *    'gb12,xpmax*wd,xd*wd,sdt0,sdnq0+sdtq0,sdt,sdnq+sdtq',
     *    gb12,xpmax*wd,xd*wd,sdt0,sdnq0+sdtq0,sdt,sdnq+sdtq,
     *    wwq,wwg,(xpmax/xd)**dels(1),gb10
        ...
        if(jcasc.ne.0)then
          gb20=(1.-xd/xpmax)**betpom(1)*sdt*(1.-fglusea(1d0,qq,1))+
     *   EsoftQZero(dble(xd/xpmax),q2dmin)*(sdnq+sdtq)*fglusea(1d0,qq,1)
        else
          gb20=EsoftQZero(dble(xd/xpmax),q2dmin)
        endif
        if(1.+2.*(-alpqua(1))+dels(1).ge.0.)then
...
5         xp=1.-(xpminl+(xpmaxl-xpminl)*rangen())**
     *    (1./(alplea(iclpro)+1.))
          if(jcasc.ne.0)then
            gb2=((1.-xd/xp)**betpom(1)*sdt*(1.-fglusea(1d0,qq,1))+
     *    EsoftQZero(dble(xd/xp),q2dmin)*(sdnq+sdtq)*fglusea(1d0,qq,1))
     *      *(xp/xpmax)**(1.+2.*(-alpqua(1))+dels(1))/gb20
          else
           gb2=EsoftQZero(dble(xd/xp),q2dmin)
     *        *(xp/xpmax)**(1.+2.*(-alpqua(1))+dels(1))/gb20
          endif
...
c            read (*,*)
          ...
          xpmaxl=xpmax**(2.+2.*(-alpqua(1))+dels(1))
          xpminl=xpmin**(2.+2.*(-alpqua(1))+dels(1))

6         xp=(xpminl+(xpmaxl-xpminl)*rangen())**
     *    (1./(2.+2.*(-alpqua(1))+dels(1)))
          if(jcasc.ne.0)then
            gb21=((1.-xd/xp)**betpom(1)*sdt*(1.-fglusea(1d0,qq,1))+
     *   EsoftQZero(dble(xd/xp),q2dmin)*(sdnq+sdtq)*fglusea(1d0,qq,1))*
     *      ((1.-xp)/(1.-xd))**alplea(iclpro)/gb20
          else
          gb21=EsoftQZero(dble(xd/xp),q2dmin)
     *         *((1.-xp)/(1.-xd))**alplea(iclpro)/gb20
          endif
...
23        zg=zgmin-rangen()*(zgmin-zgmax)
       if(rangen().gt.zg**dels(1)*((1.-xd/xp/zg)/(1.-xd/xp))**betpom(1))
     *    goto 23
...
24      call fremny(wpn,wmn,pnx,pny,sm,icp1,icp2,icp3,icp4,bx,ey0)
        ...
        if((-alpqua(1)).eq.-1.)stop'dis does not work for 1/x'
25      aks=rangen()
        z=.5*aks**(1./(1.+(-alpqua(1))))
        if(z.lt.1.e-5.or.rangen().gt.(2.*(1.-z))**(-alpqua(1)))goto 25
        ...
        if(iq1.eq.0)then
          ...
          if(jcasc.eq.0)then
            ...
            s0xh=0.
            c0xh=1.
            call psreti(nqc,jq,1,ey,0.,1.,0.,1.,s0xh,c0xh,s0h,c0h,iret)
            goto 17
          else
            gdres=(sdtq-sdsq)/4.5
            gdrga=sdrq/4.5
      ...
      si=wpi*wmi+qq
      qmin(2)=q2dmin                 !effective momentum cutoff below
      s2min=max(4.*qq,16.*q2dmin)    !mass cutoff for born scattering
...
c---------------------------------------
      ...
      if(rangen().gt.gdrga/gdres.and.si.gt.1.1*s2min+qq)then
        ...
        qqmin=max(q2dmin,s2min/(si/qq-1.))
        qqmax=min(si/2.,si-s2min)
        qmin(1)=qqmin
        xmax=1.
        xmin=(s2min+qq)/si
        if(qqmin.ge.qqmax.or.xmin.ge.xmax)stop'min>max'
        gb0=psjti(1,qmin(1),qq,si-qq,7,iqc(2),1)*psfap(qq,1.d0,0,1)
        ...
        qmin(1)=q2dmin                 !effective momentum cutoff above
        si=si-qq
        zmin=s2min/si
        dft0=psdfh4(zmin,q2dmin,qq,0,0)
     *   *psjti ( 1 ,q2dmin,qq,si,0,iqc(2),1)
     *  +(psdpdf(zmin,q2dmin,qq,0,1)+psdpdf(zmin,q2dmin,qq,0,2)+
     *  psdpdf(zmin,q2dmin,qq,0,3))*psjti ( 1 ,q2dmin,qq,si,7,iqc(2),1)

7       continue
        z=zmin**rangen()
        do i=1,4
          dfp(i)=psdfh4(z,q2dmin,qq,0,i-1)
        enddo
        dfp(1)=dfp(1)*psjti ( 1 ,q2dmin,qq,z*si,0,iqc(2),1)
        dfptot=dfp(1)
        if(iqc(2).eq.0)then
          sjq=psjti ( 1 ,q2dmin,qq,z*si,1,0,1)
          do i=2,4
            dfp(i)=dfp(i)*sjq
            dfptot=dfptot+dfp(i)
          enddo
        else
          sjqqp=psjti ( 1 ,q2dmin,qq,z*si,1,2,1)
          do i=2,4
            if(iabs(iqc(2)).eq.i-1)then
              dfp(i)=dfp(i)*(psjti ( 1 ,q2dmin,qq,z*si,1,1,1)+
     *        psjti ( 1 ,q2dmin,qq,z*si,1,-1,1))/2.
            else
              dfp(i)=dfp(i)*sjqqp
            endif
            dfptot=dfptot+dfp(i)
...
c---------------------------------------
     ...
     *  /(1.-q2ini/s2min)/2.
        sj=psjti ( 1 ,qmin(1),qq,si,iqc(1),iqc(2),1)   !total jet
        sj2=psjti1(q2dmin,qmin(1),qq,si,iqc(2),iqc(1),1)
      ...
      sjb=psboi(qmin(1),qmin(2),qq,si,iqc(1),iqc(2),1) !born parton-parton
...
c      s2max=xm0*si
      ...
      if(jj.eq.1)then
        sj0=psjti ( 1 ,qm0,qq,s2max,0,iqc(2),1)*psfap(qq,xx,iqc(1),0)+
     *  psjti ( 1 ,qm0,qq,s2max,7,iqc(2),1)*psfap(qq,xx,iqc(1),1)
        gb0=sj0/log(q2ini/qcdlam)*sngl(psuds(qm0,iqc(1)))*qm0*2.
      else
       sj0=psjti1(qm0,qmin(1),qq,s2max,0,iqc(1),1)*psfap(qq,xx,iqc(2),0)
     *  +psjti1(qm0,qmin(1),qq,s2max,7,iqc(1),1)*psfap(qq,xx,iqc(2),1)
...
c ep3 is now 4-vector for s-channel gluon produced in current ladder run
      ...
      if(jgamma.ne.1)then
        ...
        if(jj.eq.1)then
          sj1=psjti ( 1 ,q2,qq,s2,0,iqc(2),1)
          if(iqc(1).ne.0)then
            sj2=psjti ( 1 ,q2,qq,s2,iqc(1),iqc(2),1)
          elseif(iqc(2).eq.0)then
            sj2=psjti ( 1 ,q2,qq,s2,1,0,1)
          else
            sj2=psjti ( 1 ,q2,qq,s2,1,1,1)/6.+
     *      psjti ( 1 ,q2,qq,s2,-1,1,1)/6.+
     *      psjti ( 1 ,q2,qq,s2,2,1,1)/1.5
          endif
        else
...
c gb7 is the rejection function for x and q**2 simulation
        gb7=(sj1*psfap(qq,xx,iqc(jj),0)+sj2*psfap(qq,xx,iqc(jj),1))
     *  /log(qt2/qcdlam)*sngl(psuds(q2,iqc(jj)))*q2/gb0
        ...
        if(iqc(2).eq.0)then
          ...
          sj2=psjti ( 1 ,q2,qq,s2,1,1,1)/noflav(qq)/2.+
     *    psjti ( 1 ,q2,qq,s2,-1,1,1)/noflav(qq)/2.+
     *    psjti ( 1 ,q2,qq,s2,2,1,1)*(1.-1./noflav(qq))
        endif
        gb7=sj2*psfap(qq,xx,0,1)/gb0  !????*(1.-x*qq/q2)
      endif
      ...
      if(jgamma.ne.1)then
        if(rangen().lt.sj1/(sj1+sj2))then
          ...
          if(iqc(jj).ne.0)then
            ...
            jt=4
            jq=int(1.5+rangen())
            iq1=int(noflav(qq)*rangen()+1.)*(3-2*jq)
            nqc(1)=ncc(jq,jj)
            iqnew=-iq1
          endif
        endif
 ...
 27   call timsh1(q2,sngl(eprt),iq2ini,jo,ey,0.,1.,0.,1.
     .                                       ,s0xh,c0xh,s0h,c0h)
      amprt=pprt(5,1)**2
      ...
      if(s2new.gt.s2min2)then
        if(jj.eq.1)then
          gb=psjti ( 1 ,q2,qq,s2new,iqnew,iqc(2),1)
        else
...
c---------------------------------------
      ...
      qqs=max(qqs,qq)
      stop'UPDATE required!!!' !call psabor(dble(si),q2dmin,  pt_squared  ,iqc,ncc,ept,1,nptlh,bx,jtp)
      !additional argyments (Euler angles and ey boost parameters)
      goto 17
...
29    continue
     ...
     *qqmax,qqmin,xmax,xmin
      ...
      sj0=sds*psfap(qq,xx,iqc(2),0)+sdv*psfap(qq,xx,iqc(2),1)
...
c ep3 is now 4-vector for s-channel gluon produced in current ladder run
      ...
      xx=x
      sj1=sds*psfap(qq,xx,iqc(2),0)
      sj2=sdv*psfap(qq,xx,iqc(2),1)
...
c gb7 is the rejection function for x and q**2 simulation.
     ...
     *s2.lt.1.5*s2min2)then
        ...
        if(iqc(2).ne.0)then
          ...
          tu=sdn0/2.25+sdv0
          if(noflav(qq).ge.4)tu=tu*2.
          td=sdn0/9.+sdv0
          if(rangen().lt.tu/(tu+2.*td))then
            if(noflav(qq).eq.3)then
              iq1=1
            else
     ...
     *.5d0*((1.d0-x)*wpt(2)+qt2/(1.d0-x)/wpt(2)))
      ...
      ey(3)=1.
      call psdefrot(ep3,s0xh,c0xh,s0h,c0h)
 28   call timsh1(q2,sngl(eprt),iq2ini,jo,ey,0.,1.,0.,1.
     .                                       ,s0xh,c0xh,s0h,c0h)
      amprt=pprt(5,1)**2
      plprt=eprt**2-amprt-qt2
...
c---------------------------------------
     ...
     *                                                 ,qt2
        gb0=gb01+psfap(qq,xx,iqc(2),1)/log(qt2/qcdlam)
     *  *sngl(psuds(tmin,iqc(2))/psuds(tmin,1)*psuds(qq,1))
     *  /si*(1.-tmin*qq/si**2/ze)
      endif
      gb0=gb0*2.

      call psdeftr(dble(si-qq),ept,ey)
...
16    continue
      ...
      if(t.le.qq.and.long.eq.0)then
        xx=ze
        gb=psfap(qq,xx,iqc(2),1)/log(qt2/qcdlam)*sngl(psuds(t,iqc(2))
...
c        gb1=0.  !???????????????????????
     ...
     *jq,jt,iq2ini1,iq2ini2
      ...
      en1=0.5*sqrt(si-qq)
      en2=en1
      call timsh2(qq1,qq2,en1,en2,iq2ini1,iq2ini2,jo1,jo2
     *           ,ey,s0xi,c0xi,s0i,c0i  ,s0xh,c0xh,s0h,c0h,0,0,0,0,0)
      nfprt=1
      call psreti(nqc,jq,nfprt,ey,0.,1.,0.,1.,s0xh,c0xh,s0h,c0h,iret)
      ...
      nfprt=2
      call psreti(nqc,jq2,nfprt,ey,0.,1.,0.,1.,s0xh,c0xh,s0h,c0h,iret)
...
17    continue
      ...
      call psjarr(jtp,jfl)       !kinky strings formation
...
2     format(5x,i2,'-th order contribution')
      ...
      do j=1,20
        ...
        do k=1,2
          ...
          evs(i,j,l+27*(m-1)+54*(k-1))=log((ev+ev0)/psfap(qq,xx,m-1,k-1)
     *      /log(log(qq*(1.d0-xx)/qcdlam)/log(qj*(1.d0-xx)/qcdlam))*4.5)
          elseif(k.ne.1)then
            evs(i,j,l+108)=log((psev(qj,qq,xx,m,k,n)+
     *      psevi0(qj,qq,xx,2,2))/psfap(qq,xx,2,2)
     *      /log(log(qq*(1.d0-xx)/qcdlam)/log(qj*(1.d0-xx)/qcdlam))*4.5)
          endif
        enddo
        enddo
...
c-----------------------------------------------------------------------
c psdbin - DIS born cross-section (charm)
...
c for gluons, direct production of c-cb pair possible
...
c here psevi0=0 if m=2, so this part is not used for m1=0 or 1
...
c-----------------------------------------------------------------------
c psdbor - DIS born cross-section (charm)
c q1      - virtuality cutoff for current end of the ladder
c qq      - photon virtuality
c s=2(pq) - s_true + qq
c s2min   - mass cutoff for born scattering
...
c qq  - photon virtuality,
c sds - dis singlet cross-section -> dis singlet cross-section for charm
c sdn - dis nonsinglet cross-section,
c sdb - dis born cross-section for charm,
c sdt - dis singlet (charm+light)+point like+resolved(VDM) cross-section,
c sdr - dis resolved(VDM) cross-section,
...
#include "sem.h"
      ...
      sdt=0.
      sdr=0.
c charm born xsection : direct charm production without preliminary emissions
      sdb=psdbin(q2dmin,qq,s,m1,long)   

      m=min(1,iabs(m1))+1
      qlj=log(qq/q2dmin)*2.+1.
      j=int(qlj)
      if(j.lt.1)j=1
      if(j.gt.19)j=19
      wj(2)=qlj-j
      ...
      qtq=4.*max(q2dmin,qcmass**2)/(s-qq)
      if(qtq.lt.1.)then
        tmin=.5*s*qtq/(1.+sqrt(1.-qtq))
      else
        tmin=.5*s
      ...
      do k1=1,3
        k2=k+k1-1
      do j1=1,3
        sds=sds+csds(j+j1-1,k2,m+2*long)*wj(j1)*wk(k1)   !singlet charm xsection
      enddo
      enddo
      if(m.eq.1)then
...
1     continue
      ...
      do k1=1,3
        k2=k+k1-1
      do j1=1,3
        sdr=sdr+csdr(j+j1-1,k2,m)*wj(j1)*wk(k1)    !VDM resolved photon
        sdt=sdt+csdt(j+j1-1,k2,m)*wj(j1)*wk(k1)    !point-like resolved photon
      enddo
      enddo

      sdr=max(sdr,0.)
      sdt=max(sds,sds+sdt)
      sdt=sdt+sdr             !total direct charm + resolved (point-like and VDM)

2     continue
      if(long.eq.0.and.q2dmin.lt.qq.and.s.gt.qq/(1.-q2ini/qq)
     *.and.(idisco.eq.0.or.idisco.eq.1))then
        xx=qq/s
        dsi=psevi(q2dmin,qq,xx,m,2)*4.*pi**2*alfe/s
        if(m1.eq.0)then  !from gluons
c          sds=sds+dsi    !direct charm + direct light quarks from gluons
          sdt=sdt+dsi    !total + direct light quarks from gluons
        else   !from quarks
          dnsi=psevi(q2dmin,qq,xx,3,2)*4.*pi**2*alfe/s
          sdn=sdn+dnsi                 !direct light non-singlet
c          sds=sds+max(dsi-dnsi,0.)     !direct charm + direct light (singlet)
          sdt=sdt+max(dsi-dnsi,0.)     !total + direct light singlet
        endif
      endif
...
c-----------------------------------------------------------------------
      function psdvdm(qq,s,long)
c-----------------------------------------------------------------------
c psdvdm - DIS resolved cross-section (VDM) taking into account
c multiple scattering
c qq    - photon virtuality
c s     - total c.m. energy squared for the process
c long - 0 (Transverse) - 1 (Longitudinal)
c-----------------------------------------------------------------------
c      psdvdm=psdvdme(qq,s,long)     !Engel
      psdvdm=psdvdmk(qq,s,long)     !Kopeliovich

      return
      end

c-----------------------------------------------------------------------
      function psdvdme(qq,s,long)
c-----------------------------------------------------------------------
c psdvdme - DIS resolved cross-section (VDM) taking into account
c multiple scattering
c qq    - photon virtuality
c s     - total c.m. energy squared for the process
c long - 0 (Transverse) - 1 (Longitudinal)  (dummy here)
c based on Engel et al. PRD 55,6957 (1997)
c-----------------------------------------------------------------------
      common /gsigma/sigmaqq
#include "aaa.h"
#include "sem.h"
      parameter (eps=0.5,c2=2.,ampi2=0.0729) !mpi2=4*(mass pion)**2


      psdvdme=0.
      if(s.le.ampi2.or.long.eq.1)return


c Vector meson mass integration between 4*m_pi**2 and s
      psdvdme=psdvdmi(s,qq,eps,c2)
      psdvdme=psdvdme-psdvdmi(ampi2,qq,eps,c2)
c vector meson - nucleon interaction cross section from Phi(s,t)
c      print *,'psdvdme',qq,s,psdvdme,sigmaqq
      psdvdme=max(0.,psdvdme)*sigmaqq     !sigma_meson_nucleon (s,Q2)

      psdvdme=psdvdme*2.*alfe/3./pi

      return
      end

c-----------------------------------------------------------------------
      function psdvdmk(qq,s,long)
c-----------------------------------------------------------------------
c psdvdmk - DIS resolved cross-section (VDM) taking into account
c multiple scattering
c qq    - photon virtuality
c s     - total c.m. energy squared for the process
c long - 0 (Transverse) - 1 (Longitudinal)
c based on Kopeliovich et al. JETP Lett. 33, 595 (1981) 
c                         and hep-ph/0811.2024v2 (2009)
c and Mc Dermott, L. Frankfurt, V Guzey and M Strikman, Eur.Phys.J. C16,541 (2000)
c-----------------------------------------------------------------------
      common /gsigma/sigmaqq
#include "aaa.h"
#include "sem.h"
      real Nc,r2pi,lda,nu
      parameter (Nc=3)   !number of colors
c      parameter (r2pi=10.86)  !=0.65?(fm?)/0.0389(GeV?.fm?) pion in radius in GeV^-2

      dummy=s
      psdvdmk=0.
c      psdvdmk=sigmaqq*alfe
c      return
      ...
      r2pi=4*r2had(1)
      if(qq.lt.q2ini)r2pi=r2pi*qq/q2ini   !correction below q2ini

c some constants
      xmf2=(qmass(0)+(qmass(1)+qmass(2)+qmass(3)+qmass(4))/4.)**2 !m_f=<m_q>**2
      e1=exp(-1.)
      e1i=1./(1.-e1)
c nu=Q2/(2.m_p.x) is the gamma energy (Engel et al. PRD 55,6957 (1997)) 
      nu=s/(2.*amtarg)    !s=qq/x as input
      lda=qq*0.1/(0.25*qq+xmf2) !lambda from mean dipole radius
      r2Q0=min(lda/q2min,0.9*r2pi)
      delri=1./(r2pi-r2Q0)
      
c integaration over alpha from 0 to 1 (momentum fraction between quarks)
      do i1=1,ig1
       do m1=1,2
         alp1=0.25*(.5+tgss(ig1,i1)*(m1-1.5))   !integration form 0 to 0.25
                                       !change variable to alp1=alpha*(1.-alpha)
         alp2=alp1**2
         rqq=0.1/(alp1*qq+xmf2)  !average dipole radius (10. from tests)
c integaration over rT from 0 to inf (distance between quarks)
         phi2a=0.         
         do i2=1,ig2
          do m2=1,2
            z=.5+tgss(ig2,i2)*(m2-1.5)   !integration form 0 to 1.
            zv1=exp(-z)
            zv2=(e1*z)
c change variable to r1=sqrt(rqq*z) from 0 to sqrt(rqq)
            r21=-rqq*log(zv1)
            r01=sqrt(r21)
c suppression function for small radius (correponds to point like interaction)
c equations (11) to (13) in McDermott et al.
            if(r21.lt.r2Q0)then
              H11=0.
            else
c life time of the dipole : tau=2*nu/(Q2+M2) where M2~4*(xmf2+0.75*lda/r2) is the dipole mass
c (McDermott et al.)
              am2dip=4.*(xmf2+0.75*lda/r21)
              tau=(2.*nu/(qq+am2dip))**2/r2had(icltar)
              H11=min((r21-r2Q0)*delri,1.)
              H11=e1i*(1.-exp(-H11))*exp(-1./tau)
            endif
c            H11=exp(-1./tau)
c change variable to r2=sqrt(-rqq*log(z*exp(-1))) from sqrt(rqq) to inf
            r22=-rqq*log(zv2)
            r02=sqrt(r22)
c suppression function for small radius (correponds to point like interaction)
c equations (11) to (13) in McDermott et al.
...
c life time of the dipole : tau=2*nu/(Q2+M2) where M2~4*(xmf2+0.75*lda/r2) is the dipole mass
c (McDermott et al.)
              am2dip=4.*(xmf2+0.75*lda/r22)
              tau=(2.*nu/(qq+am2dip))**2/r2had(icltar)
              H12=min((r22-r2Q0)*delri,1.)
              H12=e1i*(1.-exp(-H12))*exp(-1./tau)
            endif
c            H12=exp(-1./tau)
c            H11=1.
c            H12=1.
            phi2r=0.
            do nf=1,3           ! loop on 3 ligth flavors
              qmf2=(qmass(0)+qmass(nf))**2
              eps2=alp1*qq+qmf2
              eps=sqrt(eps2)
              r1=r01*eps
              r2=r02*eps
              if(long.eq.0)then
                phi21=(1.-2.*alp1)*eps2*bessk1(r1)**2
                phi22=(1.-2.*alp1)*eps2*bessk1(r2)**2
                phi21=phi21+qmf2*bessk0(r1)**2
                phi22=phi22+qmf2*bessk0(r2)**2
                phi2=2.*(phi21*H11+phi22*H12/z)
              else
                phi21=bessk0(r1)**2
                phi22=bessk0(r2)**2
                phi2=8.*qq*alp2*(phi21*H11+phi22*H12/z)
              endif
              if(nf.eq.1)then   ! u quark charge (2/3)**2
                phi2r=phi2r+phi2/2.25
              else              ! d,s quark charge (-1/3)**2
                phi2r=phi2r+phi2/9.
              endif
            enddo
            phi2a=phi2a+wgss(ig2,i2)*phi2r
c           print *,s,qq,r1,H11,r22,H12,phi2r,phi2a
          enddo
         enddo
         phi2a=phi2a*pi*rqq/2.     !common factors 2.*pi because of int_d2b=2.pi.b.db, rqq/2 because of r->z, /2. because z is from -1 to 1 with Gauss but we need 0 to 1.
         psdvdmk=psdvdmk+wgss(ig1,i1)*phi2a/sqrt(1.-4.*alp1)       !Jacobian
       enddo
      enddo

      psdvdmk=psdvdmk/4.        !1./8 gauss integration *2 from change in variables

c vector meson - nucleon interaction cross section
      psdvdmk=max(0.,psdvdmk)*sigmaqq    !sigma_qqb_nucleon (s,Q2)

      psdvdmk=Nc*alfe/(2.*pi)**2*psdvdmk
c      print *,'psdvdmk',qq,qq/s,s,long,psdvdmk,sigmaqq

      return
      end

c-----------------------------------------------------------------------
      subroutine psdsig(qq,s)
c-----------------------------------------------------------------------
c psdvdm - DIS resolved cross-section (VDM) taking into account
c multiple scattering
c qq    - photon virtuality
c s     - total c.m. energy squared for the process
c Note : paramini has to be called before for s and iclpro=1
c based on Engel et al. PRD 55,6957 (1997)
c-----------------------------------------------------------------------
      common /ar3/  x1(7),a1(7)
      common /gsigma/sigmaqq
#include "aaa.h"
#include "sem.h"
      dimension rpom0(idxD0:idxD1)


      xmf2=(qmass(0)+(qmass(1)+qmass(2)+qmass(3)+qmass(4))/4.)**2 !m_f=<m_q>**2
c vector meson - nucleon interaction cross section from Phi(s,t)
      r2gam=0.025/(0.25*qq+xmf2) !dipole radius (0.025=1./4./10.)
      rs=r2gam+r2had(icltar)+slopom*log(s)+max(r2hads(1),r2hads(2))
      ...
      do i=0,imax
        if(i.eq.2)then
          rs0=r2had(iclpro)+r2had(icltar)+slopom*log(s)
     .       +max(r2hads(1),r2hads(2))
          rpom0(i)=rs0/(r2gam+r2had(icltar)+slopom*log(s)
     .       +max(r2hads(1),r2hads(2)))
        else
          rs0=r2had(iclpro)+r2had(icltar)+slopom*log(s)
          rpom0(i)=rs0/(r2gam+r2had(icltar)+slopom*log(s))
      ...
      do i1=1,7
        do m=1,2
          ...
          do i=0,imax
            zp=0.
            zt=0.
            call Gfunpar(zp,zt,0.,0.,2,i,b1,s,alp,bet,betp,epsp,epst
     .                  ,epss,gamvv)
            vvv11=vvv11+alp*rpom0(i)*zv1**(rpom0(i)*rpom)
c            print *,'b1',b1,rpom0(i)*zv1**(rpom0(i)*rpom)
          enddo
          do i=0,imax
            zp=xpar98
            zt=xpar99
            call Gfunpar(zp,zt,0.,0.,2,i,b2,s,alp,bet,betp,epsp,epst
     .                  ,epss,gamvv)
            vvv12=vvv12+alp*rpom0(i)*zv2**(rpom0(i)*rpom)
c            print *,'b2',b2,rpom0(i)*zv2**(rpom0(i)*rpom)
          enddo

          ww11=1.d0-exp(-0.5*vvv11)
          ww12=1.d0-exp(-0.5*vvv12)
          G0=G0+a1(i1)*(ww11+ww12/z)
        enddo
        enddo
        sigmaqq=2.*G0*pi*rpom/2./.0389    !common factor (2 because of sigtot, 2.*pi because of int_d2b=2.pi.b.db, rpom/2 because of b->z, /0.0389 to have GeV-2 and /2. because z is from -1 to 1 with Gauss but we need 0 to 1.

c        sigmaqq=sigmaqq/(1.+qq/0.59)**2    !gamma PDF suppression factor
c        print *,'sigmaqq',qq,s,qq/s,r2gam,rpom0(1),sigmaqq*.0389*10.   !(sig in mb)

      return
      end

c-----------------------------------------------------------------------
      subroutine test(qq,long)
c-----------------------------------------------------------------------
c test - integration over alpha of |psi|?
c qq    - photon virtuality
c long - 0 (Transverse) - 1 (Longitudinal)
c based on Kopeliovich et al. JETP Lett. 33, 595 (1981) 
c                         and hep-ph/0811.2024v2 (2009)
...
c some constants
      xmf2=(qmass(0)+(qmass(1)+qmass(2)+qmass(3))/3.)**2    !m_f=<m_q>**2
      e1=exp(-1.)
      e1i=1./(1.-e1)
      
         rqq=1./(0.25*qq+xmf2)  !dipole radius
         do i2=1,ig2
          do m2=1,2
            z=.5+tgss(ig2,i2)*(m2-1.5)   !integration form 0 to 1.
            zv1=exp(-z)
         ...
         phi2r2=0.         
            do nf=1,3           ! loop on 3 ligth flavors
              ...
              if(long.eq.0)then
                phi21=(1.-2.*alp1)*eps2*bessk1(r1)**2
                phi22=(1.-2.*alp1)*eps2*bessk1(r2)**2
                phi21=4.*pi*r01*(phi21+qmf2*bessk0(r1)**2)
                phi22=4.*pi*r02*(phi22+qmf2*bessk0(r2)**2)
              else
                phi21=8.*qq*alp2*bessk0(r1)**2
                phi22=8.*qq*alp2*bessk0(r2)**2
              endif
              if(nf.eq.1)then   ! u quark charge (2/3)**2
                phi2r1=phi2r1+phi21/2.25
                phi2r2=phi2r2+phi22/2.25
              else              ! d,s quark charge (-1/3)**2
                phi2r1=phi2r1+phi21/9.
                phi2r2=phi2r2+phi22/9.
              endif
            enddo
         test1=test1+wgss(ig1,i1)*phi2r1/sqrt(1.-4.*alp1)       !Jacobian
         test2=test2+wgss(ig1,i1)*phi2r2/sqrt(1.-4.*alp1)       !Jacobian
          enddo
         enddo
      test1=test1/4.        !1./8 gauss integration *2 from change in variables
      test1=Nc*alfe/(2.*pi)**2*test1
      test2=test2/4.        !1./8 gauss integration *2 from change in variables
      test2=Nc*alfe/(2.*pi)**2*test2
      if(long.eq.0)then
      write(10,*)r01*sqrt(0.0389),test1
      write(10,*)r02*sqrt(0.0389),test2
      else
      write(11,*)r01*sqrt(0.0389),test1
      write(11,*)r02*sqrt(0.0389),test2
      endif
...
c-----------------------------------------------------------------------
      function psdvdmi(am2,qq,eps,c2)
c-----------------------------------------------------------------------
c psdvdmi - integrant for mass factor of vector meson cross section
c qq    - photon virtuality
c s     - total c.m. energy squared for the process
c based on Engel et al. PRD 55,6957 (1997)
c-----------------------------------------------------------------------
#include "aaa.h"
      am2qq=am2+qq
      psdvdmi=c2*qq*(1.-eps)/am2qq
      psdvdmi=psdvdmi+(qq*(1.-eps)+c2)*log(am2qq/(am2qq+c2))
      psdvdmi=psdvdmi/c2**2
      return
      end
...
c-----------------------------------------------------------------------
c psdrga - DIS resolved cross-section (VDM)
c qq    - photon virtuality
c s     - total c.m. energy squared for the process
c s2min - mass cutoff for born scattering
...
#include "sem.h"
      ...
      do m=1,2
        z=xmin**(.5+(m-1.5)*x1(i))
c in the followinf psdfh4(,,,0,) is used because pdf of gamma not defined in psdpdf in general
        tu=psdfh4(z,q2dmin,qq,0,1)
        td=psdfh4(z,q2dmin,qq,0,2)
        ts=psdfh4(z,q2dmin,qq,0,3)
        tg=psdfh4(z,q2dmin,qq,0,0)
        if(j.eq.0)then
          sj=0  ! wrong use of psjti !!!    tg*psjti ( 1 ,q2dmin,qq,z*s,0,j,1)+
c     *    (tu+td+ts)*psjti ( 1 ,q2dmin,qq,z*s,1,j,1)
        else
          sj=0    !tg*psjti ( 1 ,q2dmin,qq,z*s,0,j,1)+
c     *    (tu+td)*(psjti ( 1 ,q2dmin,qq,z*s,1,1,1)/4.+
c     *    psjti ( 1 ,q2dmin,qq,z*s,-1,1,1)/4.+
c     *    psjti ( 1 ,q2dmin,qq,z*s,2,1,1)/2.)+
c     *    ts*psjti ( 1 ,q2dmin,qq,z*s,2,1,1)
        endif
...
c-----------------------------------------------------------------------
c psdres - DIS resolved photon cross-section (point like)
c qq    - photon virtuality
      ...
      do m=1,2
        ...
        if(zmax.gt.zmin)then
          ...
          do m1=1,2
            ...
            xx=z
            if(j.eq.0)then
              sj=0 ! wrong use of psjti !!!   psfap(qq,xx,0,1)*psjti ( 1 ,qi,qq,s2,1,j,1)
            else
              sj=0 ! wrong use of psjti !!psfap(qq,xx,0,1)*(psjti ( 1 ,qi,qq,s2,1,1,1)/6.+
c     *        psjti ( 1 ,qi,qq,s2,-1,1,1)/6.+
c     *        psjti ( 1 ,qi,qq,s2,2,1,1)/1.5)
            endif
...
c-----------------------------------------------------------------------
c psds - DIS singlet cross-section for c-quark
c q1      - virtuality cutoff for current end of the ladder
c qq      - photon virtuality
c s=2(pq) - s_true + qq
c s2min   - mass cutoff for born scattering
...
c-----------------------------------------------------------------------
c psdsj - integrand for dis singlet cross-section for charm quarks
c q1 - virtuality cutoff for current end of the ladder
...
#include "sem.h"
      ...
      fb=psdbom(s,t,u,qq,long)    !c-quark matrix element squared
...
c-----------------------------------------------------------------------
c psdsin - DIS singlet cross-section interpolation for c-quark
c q1 - effective momentum cutoff for current end of the ladder,
c qq - photon virtuality,
c s -  total c.m. energy squared for the ladder,
c m1 - parton type at current end of the ladder (0 - g, 1,2 - q)
