...
c  usage:
c
c      iseqini=2  !should be less than kseq in RMMARD (=2 in EPOS)
c      iseqsim=1  !.ne.iseqini : sequence number at start program
c      seedi=234567d0
c      seedj=543221d0
c      call ranfini(0d0,0,-1) !initialize some parameters
c      call ranfini(seedi,iseqini,1)
c      call ranfini(seedj,iseqsim,2)
c      .....
c      x=rangen()
c
c-----------------------------------------------------------------------
c
c  To rerun the program staring from an event N > 1, use  (for example)
c
c    set seedj 621472000
c    set seedf 0.2221090166000000D+11
c
c  using the values for seedj and seedf printed out in the run in question.
c
c-----------------------------------------------------------------------
...
#include "aaa.h"
      ...
      dummy=0d0
 1    rangen=sngl(dranf(dummy))
      if(rangen.le.0.)stop'\n\n STOP in rangen: zero value\n\n'
...
#include "aaa.h"
      ...
      double precision function rangenxx()
      rangenxx=rangen()
      end
