...
//     check that it has a good PDG status
	...
	|| status[0]=='S' || status[0]=='F' || ccharge!=0  || bcharge!=0  || tcharge!=0) {
      fStatus[fNParticles] = kFALSE;
      goodStatusParticles--;
    }

    //cout << fNParticles << setw(19) << fParticles[fNParticles]->GetName() << setw(12) << fParticles[fNParticles]->GetPDG() <<  setw(12)   <<  fStatus[fNParticles]  << setw(12) << status[0] << endl;
	...
	if(fParticles[fNParticles-1]->GetBeauty()!=0)
        ...
        //cout << fNParticles << setw(19) << fParticles[fNParticles]->GetName() << setw(12) << fParticles[fNParticles]->GetPDG() <<endl;
  ...
  SortParticles();
  //cout << "Particle definitions found : " << fNParticles << endl
...
Int_t DatabasePDG2::GetKParticles(Bool_t all) {
  if(all)
    return fNParticles;

  Int_t nKGoodStatus = 0;
  for(Int_t i=0; i<fNParticles; i++)
    if(fStatus[i]) nKGoodStatus++;
    else if(fParticles[i]->GetCharm()!=0) nKGoodStatus++;
  return nKGoodStatus;
}
...
{
  if(abs(pdg)==211 || abs(pdg)==111 || abs(pdg)==2112 || abs(pdg)==2212 || abs(pdg)==311 || abs(pdg)==321 || abs(pdg)==3212) return true ;
  else return false ;
}
...
void DatabasePDG2::DumpDecays(){
 ofstream foutdec ("z-decays.dump");
 for(int i=0; i<fNParticles; i++){
   ParticlePDG2 *part = GetPDGParticleByIndex(i);
   for(int id=0; id<part->GetNDecayChannels(); id++){
    DecayChannel* dc = part->GetDecayChannel(id);
    int nd = dc->GetNDaughters();
    int d [3] = {0, 0, 0};
    for(int i=0; i<nd; i++) d[i] = dc->GetDaughters()[i];
    foutdec << setw(16) << part->GetPDG() << setw(16) << d[0]
     << setw(16) << d[1] << setw(16) << d[2]
     << setw(16) << dc->GetBranching() << endl;
   }
 }
 foutdec.close();
}
